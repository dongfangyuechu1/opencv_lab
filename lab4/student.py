...
def compute_photometric_stereo_impl(lights, images):
    ...
    (number, height, width, z) = arr_i.shape
    albedo = np.float32(np.zeros((height, width, z)))
    normals = np.float32(np.zeros((height, width, 3)))
    _normal = np.float32(np.zeros([3, 1]))
    for i in range(height):
        for j in range(width):
            for k in range(z):
                I = arr_i[:, i, j, k].reshape(number, 1)
                G = np.dot(np.linalg.inv(np.dot(lights.T, lights)), np.dot(lights.T, I)).reshape(3, 1)
                k_d = np.linalg.norm(G, ord=None, axis=None, keepdims=False)
                if k_d < 1e-7:
                    k_d = 0
                    _normal = G.reshape(3)
                else:
                    _normal = (G / k_d).reshape(3)
                albedo[i, j, k] = k_d
            normals[i, j, 0] = _normal[0]
            normals[i, j, 1] = _normal[1]
            normals[i, j, 2] = _normal[2]
...
def project_impl(K, Rt, points):
    ...
    #raise NotImplementedError()
    p_m = np.dot(K, Rt)
    (height, width, z) = points.shape
    projections = np.zeros((height, width, 2))
    for i in range(height):
        for j in range(width):
            x_p = points[i, j, 0]
            y_p = points[i, j, 1]
            z_p = points[i, j, 2]  #points of x,y,z
            X = np.array([x_p, y_p, z_p, 1]).reshape(4, 1)
            u = np.dot(p_m, X)
            u = u / u[2, 0]
            projections[i, j, 0] = u[0, 0]
            projections[i, j, 1] = u[1, 0]
    return projections
...
def preprocess_ncc_impl(image, ncc_size):
    ...
    number = ncc_size ** 2
    normalized = np.zeros((height, width, (channels * number)))
    _size = ncc_size // 2 #his must use //
    for i in range(height):
        for j in range(width):
            if i - _size >= 0 and j - _size >= 0 and i + _size < height and j + _size < width:
                pre_patch = image[(i - _size):(i + _size + 1), (j - _size):(j + _size + 1), :]\
                    .reshape((number, channels))
                means = np.mean(pre_patch, axis=0)
                patch = pre_patch - means
                sum_ = np.sum(patch ** 2)
                _patch = np.sqrt(sum_)
                if _patch == 0:
                    normalized[i, j, :] = patch.T.reshape(1, 1, -1)
                else:
                    std_patch = patch / _patch
                    normalized[i, j, :] = std_patch.T.reshape(1, 1, -1)
                numb = np.linalg.norm(normalized[i, j, :])
                if numb < 1e-6:
                    normalized[i, j, :] = 0
            else:
                normalized[i, j, :] = 0
...
def compute_ncc_impl(image1, image2):
    ...
    for i in range(height):
        for j in range(width):
            point = image1[i, j, :] * image2[i, j, :]
            ncc[i, j] = np.sum(point)
    return ncc
        ...
        for i in range(height):
            for j in range(width):
                if alpha[i, j] != 0:
                    ...
                    d = depth_weight * depth[i, j]
                    b.append(d)
                    row += 1
    if normals is not None:
        for i in range(height):
            for j in range(width):
                if alpha[i, j] != 0 and (i + 1 < height and j + 1 < width and
